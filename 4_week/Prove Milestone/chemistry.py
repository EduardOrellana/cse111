#Copyright @Erick Orellana Online Student of BYU Idaho
...
def make_periodic_table():
    '''The compound list must contain all the data in
    the table of elements shown in the Table of 
    ...
    #For the exceeding Requirement 3, I added the total of protons inside the periodic table, I found the information on Internet.
    #http://hobart.k12.in.us/ksms/PeriodicTable/energy%20levels.htm
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    #Convert the Parameters into Variables
    TABLE_PERIODIC = periodic_table_dict
    TABLE_SYMBOL = symbol_quantity_list
    VALUE_RETURNED = 0

    for i in TABLE_SYMBOL:
        #EXTRACT EACH ELEMENT OF SYMBOL_QUANITYT_LIST
        SYMBOL_FINDED = i[SYMBOL_INDEX]
        SYMBOL_QUANTITY = i[QUANTITY_INDEX]

        #Pull out the specif symbol we are looking for.
        ATOMIC = TABLE_PERIODIC[SYMBOL_FINDED]

        #Pull our from the list of the dictionary the mass.
        ATOMIC_MASS = ATOMIC[ATOMIC_MASS_INDEX]

        OPERATOR = ATOMIC_MASS * SYMBOL_QUANTITY

        VALUE_RETURNED += OPERATOR
...
def main():
    ...
    #Declare and call the funcion make periodic table

    MOLECULAR_FORMULA_SAMPLE = str(input('\nEnter the molecular formula of the sample: \n'))
    MASS_IN_GRAMS_SAMPLE = float(input('\nEnter the mass in grams of the sample: \n'))
    ...
    LIST_SYMBOLS = parse_formula(MOLECULAR_FORMULA_SAMPLE, PERIODIC_TABLE)


    # Call the compute_molar_mass function to compute the
    # molar mass of the molecule from the compound list.
    MOLAR_MASS = compute_molar_mass(LIST_SYMBOLS,PERIODIC_TABLE)

    NUMBERS_OF_MOLES = MASS_IN_GRAMS_SAMPLE / MOLAR_MASS


    #Exceeding Requirement 2 & 3 Call the get_formula_name to discover if the formula name is in the known_molecules_dict

    NAME_FORMULA = get_formula_name(MOLECULAR_FORMULA_SAMPLE,known_molecules_dict)

    TOTAL_PROTONS = sum_protons(LIST_SYMBOLS,PERIODIC_TABLE)

    print(f'\n{MOLAR_MASS} grams/moles\n')
    print(f'{NUMBERS_OF_MOLES:.5f} moles')
    print(f'\nThe name according our dictionary is: {NAME_FORMULA.capitalize()}')
    print(f'\nTotal of PROTONS: {TOTAL_PROTONS}\n\n')
    


#Exceeding Requirement 1
...
def get_formula_name(formula, known_molecules_dict):
    ...
    if formula in known_molecules_dict:
        ...
        VALUE = "That formula we don't have into our dictionary."
...
def sum_protons(symbol_quantity_list, periodic_table_dict):
    ...
    for i in symbol_quantity_list:
        ...
        PROTON = periodic_table_dict[SYMBOL_FINDED]

        PROTONS = PROTON[PROTONS_INDEX]

        OPERATOR = SYMBOL_QUANTITY * PROTONS
...
#To be certain to protecth the call to main with and if statement as taught.
