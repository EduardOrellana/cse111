...
def test_suffix():
    """Verify that the suffix functions workds correctly,
    parameters: none
    return nothing
    """

    suff = suffix('found', 'profound')
    assert isinstance(suff, str), 'Suffix function must return a strign'

    #Call de suffix function nine times and use an assert
    #Statement to verify that the string returned by the
